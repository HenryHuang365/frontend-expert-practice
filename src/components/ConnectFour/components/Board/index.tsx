...
const INITIAL_STATE: IConnectFourBoardState = {
  ...
  isGameOver: false,
  board: createListOfSize(CONNECT_FOUR_CONFIG.MAX_COLS).map(() => createListOfSize(CONNECT_FOUR_CONFIG.MAX_ROWS).map(() => null)),
};
...
const Board: React.FC<Props> = () => {
  ...
  const hasPlayerWon = useCallback(
    (col: number, row: number, colStep: number, rowStep: number, player: number, currBoard: Array<Array<number | null>>) => {
      let consecutiveMoves = 0;
      let currCol = col;
      let currRow = row;

      while (currCol < CONNECT_FOUR_CONFIG.MAX_COLS && currRow < CONNECT_FOUR_CONFIG.MAX_ROWS && currBoard[currCol][currRow] === player) {
        consecutiveMoves++;
        currCol += colStep;
        currRow += rowStep;
      }
  ...
  const handleTileClick = useCallback(
    (colIndex: number) => {
      ...
      const currBoard: Array<Array<number | null>> = JSON.parse(JSON.stringify(board));
      ...
      const hasWonDiagonally =
        hasPlayerWon(colIndex, lastEmptyCell, 1, 1, currPlayer, currBoard) || hasPlayerWon(colIndex, lastEmptyCell, 1, -1, currPlayer, currBoard);
  ...
  return (
    <div className={classes.game}>
      {isGameOver && (
        <h1 className={classes.title}>Player {isPlayerOneTurn ? CONNECT_FOUR_CONFIG.PLAYERS.P1 : CONNECT_FOUR_CONFIG.PLAYERS.P2} Wins</h1>
      )}
          ...
          <div className={classes.column} key={`column-${colIndex}`}>
            {column.map((item, index) => (
              <div
                ...
                key={`tile-${colIndex}-${index}`}
                onClick={() => {
                  handleTileClick(colIndex);
                }}
              >
      ...
      {isGameOver && (
        <ButtonWhite
          onClick={() => {
            dispatch({ type: ACTION_TYPE.RESET_BOARD });
          }}
        >
          Restart
        </ButtonWhite>
